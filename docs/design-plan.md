...
> **Status (live, v0.9.6).** Feature-complete for v1.0 except music; rigged Mixamo zombies and all asset work shipped. M0 / M1 / M2 / M3 done end-to-end, plus a deep bug audit (v0.9.5) that cleared the error console and fixed the zombie visual-feedback pipeline (headshot flash, Director rage recolor, death animation timing, tween races).
>
> **In the build:** 6 weapons (Quaternius CC0 GLBs, -90° Y orientation, placeholder hand + trigger-pull animation, card-driven visual accessories: grip / extended mag / muzzle brake / sight), rigged Mixamo zombie with Walk/Attack/Death animations + per-archetype tinting, 7 enemy archetypes including Armored Walker, 2 bosses (Subject + Director with working phase-2 rage), 38 cards incl. 3 synergies + 3 curses, targeted card-buy shop (CardBrowseScreen) alongside the random draft, 26 challenges across 4 tiers, daily seeded challenge, 4 run modifiers, token shop with 9 offers + redesigned auto-turret (pedestal + rotating yoke + LED), lifetime stats / challenges browser / credits / modifiers / content-warning screens, story intro (pauses tree, one-time), intercom flavor captions, two arenas with boss-wave red lighting + 7 randomized sci-fi debris props (~4.3 MB total 3D assets after 1K texture downscale), full settings (sensitivity, FOV, volumes, gore, fullscreen, crosshair style/size/color, FPS counter, mouse smoothing, tutorial replay, colorblind mode, save export/import), dev console (~), pooled VFX (Tracer/BulletHole/BloodBurst/Casing), 3-slot save backups, hold-to-confirm destructive actions, weapon inspect (hold I), gamepad rumble.
>
> **Audio (v0.9 restart):** intentionally minimal — player fire/reload synths, UI sounds, streak-break, barrier-destroyed thud. No ambient loops, no zombie vocals, no positional 3D. AudioMan has zero EventBus listeners; every sound is an explicit call.
>
> **Verification:** `tools/smoke_test.tscn` — 42 headless assertions, green. See **`docs/production-gaps.md`** for the punch list and **`docs/INDEX.md`** for the doc map.
