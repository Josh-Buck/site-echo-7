...
- [x] **Hand on weapon viewmodel** ✓ shipped (placeholder primitives) v0.9.4. Weapon.tscn now ships a Hand node — forearm (sleeve green), palm (skin tone), thumb wrapping the top of the grip, four fingers along the trigger side. Index finger (Finger1) pulls back on _kick() and springs out via _update_trigger_pull(). Primitives, not a rigged FPS hand — iterate when a real rigged hand asset lands.
